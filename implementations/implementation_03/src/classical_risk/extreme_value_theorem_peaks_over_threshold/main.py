...
- Data loading
- Returns computation
- Rolling VaR and CVaR calculation via EVT-POT methodology
- Backtesting
- Metrics computation
- Report generation
"""
...
from .evt_calculator import (
    compute_rolling_var_from_evt,
    compute_rolling_cvar_from_evt,
    align_returns_and_var,
    align_returns_and_cvar,
    select_threshold_automatic,
    fit_gpd_mle
)
...
def _save_restructured_json(
    ...
    output_data = {
        'metadata': {
            'task': 'evt_pot_var_cvar_backtesting',
...
def _process_single_portfolio(
    ...
    results = []
    evt_runtimes = []  # Track EVT fitting time per configuration
    threshold_runtimes = []  # Track threshold selection time per configuration
    ...
    for confidence_level in confidence_levels:
        for horizon in horizons:
            for window in estimation_windows:
                try:
                    # Track time for EVT fitting and threshold selection
                    evt_start_time = time.time()
                    
                    # Compute rolling VaR and CVaR from EVT
                    rolling_var = compute_rolling_var_from_evt(
                        portfolio_returns,
                        window=window,
                    ...
                    rolling_cvar = compute_rolling_cvar_from_evt(
                        portfolio_returns,
                        rolling_var,
                        window=window,
                        confidence_level=confidence_level,
                    ...
                    evt_runtime = time.time() - evt_start_time
                    evt_runtimes.append(evt_runtime)
                    threshold_runtimes.append(evt_runtime * 0.3)  # Approximate threshold selection time
                    ...
                    # Compute EVT-specific metrics (need to fit GPD once more for metrics)
                    # Use a representative window to get GPD parameters
                    try:
                        window_returns = portfolio_returns.iloc[-window:] if len(portfolio_returns) >= window else portfolio_returns
                        if automatic_threshold:
                            threshold, _, _ = select_threshold_automatic(
                                window_returns,
                                threshold_quantiles,
                                min_exceedances
                            )
                        else:
                            from .evt_calculator import select_threshold_quantile
                            threshold, _ = select_threshold_quantile(
                                window_returns,
                                threshold_quantiles[0] if threshold_quantiles else 0.95,
                                min_exceedances
                            )
                        
                        from .evt_calculator import extract_exceedances
                    ...
                    result = {
                        ...
                        'var_runtime_ms': evt_runtime * 1000,
                        'evt_fitting_time_ms': evt_runtime * 1000,
                        'threshold_selection_time_ms': (evt_runtime * 0.3) * 1000,
                        **accuracy_metrics,
                        **tail_metrics,
                        **cvar_tail_metrics,
                        **evt_tail_metrics,
    ...
    evt_runtime_total = sum(evt_runtimes) if len(evt_runtimes) > 0 else 0.0
    
    return results, evt_runtime_total, aligned_data
...
def evaluate_evt_pot_var_cvar(
    ...
    """
    Main function to evaluate VaR and CVaR using EVT-POT methodology for multiple portfolios.
    ...
    # Get EVT settings
    confidence_levels = evt_settings.get('confidence_levels', [0.95, 0.99, 0.995])
    horizons = evt_settings.get('horizons', [1, 10])
    estimation_windows = evt_settings.get('estimation_windows', [252, 500, 750])
    threshold_settings = evt_settings.get('threshold_selection', {})
    threshold_quantiles = threshold_settings.get('quantiles', [0.90, 0.95, 0.97])
    automatic_threshold = threshold_settings.get('automatic_threshold', True)
    ...
    print(f"  Threshold selection: {'automatic' if automatic_threshold else 'quantile'}")
    ...
    print(f"  Note: EVT-POT fitting is computationally intensive. This may take several minutes...")
    ...
    if len(runtimes) > 0:
        evt_runtime_total = sum(runtimes)
        avg_evt_runtime_per_portfolio = evt_runtime_total / num_portfolios if num_portfolios > 0 else 0
        print(f"  EVT fitting/forecast runtime: {evt_runtime_total/60:.2f} minutes ({evt_runtime_total:.2f} seconds)")
        print(f"  Average EVT runtime: {avg_evt_runtime_per_portfolio*1000:.2f} ms per portfolio")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Evaluate EVT-POT VaR/CVaR for portfolios'
    )
    parser.add_argument(
        '--config',
        type=str,
